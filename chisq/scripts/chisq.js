	...
	$('#dfbox').keyup(function() {
		...
		if (str.length > 0) {
			if (isNaN(str)) {
				...
				if (parseInt(str) == 0) {
					alert('Degree of freedom cannot be zero.');
					$(this).val(_df);
					return;
				} 
				if (parseInt(str) == 1) return;
				...
				if (_df < 20)
					...
					BLO = Math.round(_df / 2) - 10;
				BHI = Math.round(_df * 1.5) + 12;
